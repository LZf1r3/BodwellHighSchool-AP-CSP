...
def draw_northern_lights():
    """Draw aurora borealis effect"""
    t.penup()
    colors = ['#00ff87', '#60efff', '#00d4aa', '#7bed9f']
    ...
    for wave in range(3):
        ...
        for x in range(-500, 501, 20):
            t.goto(x, y_start + random.randint(-30, 30))
            t.pendown()
            t.width(random.randint(2, 6))
            for _ in range(3):
                t.goto(x + 20, y_start + random.randint(-35, 35))
        t.penup()
